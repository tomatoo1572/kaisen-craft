...
func get_character_appearance() -> Dictionary:
	...
	return {
		...
		"hair_data": KZ_HairStyleLibrary.build_preset("soft_messy"),
		"clothing": {}
	}
...
func set_body_build(build: String) -> void:
	var profile: Dictionary = get_character_appearance()
	profile["body_preset"] = build.to_lower()
	apply_character_appearance(profile)

func set_body_size(height_scale_value: float, width_scale_value: float, _body_weight_value: float) -> void:
	var profile: Dictionary = get_character_appearance()
	profile["height_scale"] = height_scale_value
	profile["height_meters"] = 1.8288 * height_scale_value
	profile["width_scale"] = clampf(width_scale_value, 0.82, 1.18)
	apply_character_appearance(profile)
...
func _apply_camera_mode() -> void:
	...
	var eye_forward_offset: float = 0.0
	var first_person_visual_offset: Vector3 = Vector3.ZERO
	var first_person_camera_local: Vector3 = Vector3(0.0, eye_height, eye_forward_offset)
	...
	if player_model != null:
		...
		if player_model.has_method("get_first_person_camera_local_pos"):
			first_person_camera_local = player_model.call("get_first_person_camera_local_pos")
		else:
			first_person_camera_local = Vector3(0.0, eye_height, eye_forward_offset)
	...
	if camera_mode == 0:
		...
		cam.position = first_person_camera_local
