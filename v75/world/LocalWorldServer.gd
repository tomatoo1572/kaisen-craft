...
func get_height_at_world(wx: int, wz: int) -> int:
	if _main_noise == null:
		_rebuild_main_noise()
	return _noise_height(_main_noise, wx, wz, terrain_base_height, terrain_height_scale, dims.y)
...
func _noise_height(noise: FastNoiseLite, wx: int, wz: int, base_h: int, scale_h: int, max_y: int) -> int:
	var macro: float = noise.get_noise_2d(float(wx) * 0.30, float(wz) * 0.30)
	var detail: float = noise.get_noise_2d(float(wx), float(wz))
	var ridge: float = absf(noise.get_noise_2d(float(wx) * 1.65, float(wz) * 1.65))
	var plains_bias: float = clampf((macro + 1.0) * 0.5, 0.0, 1.0)
	var rolling: float = detail * float(scale_h) * lerpf(0.20, 0.52, plains_bias)
	var continental: float = macro * float(scale_h) * 0.88
	var hill_boost: float = maxf(0.0, detail - 0.18) * float(scale_h) * 0.55
	var valley_soften: float = ridge * 3.0
	var h: int = int(round(float(base_h) + continental + rolling + hill_boost - valley_soften))
