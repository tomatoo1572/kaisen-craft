...
var inventory_preview_container: SubViewportContainer
var inventory_preview_viewport: SubViewport
var inventory_preview_root: Node3D
var inventory_preview_camera: Camera3D
var inventory_preview_model: Node3D
var equipment_slot_buttons: Array[KZ_SlotButton] = []
var character_body_texture_option: OptionButton
var character_skin_color_button: ColorPickerButton
var character_hair_style_option: OptionButton
var character_hair_color_button: ColorPickerButton
...
var character_shape_triangle: Control
var character_hair_editor_panel: Control
...
func _build_ui() -> void:
	...
	inv_panel.offset_left = -430
	inv_panel.offset_right = 430
	inv_panel.offset_top = -215
	inv_panel.offset_bottom = 215
	...
	var inv_outer := HBoxContainer.new()
	inv_outer.add_theme_constant_override("separation", 12)
	inv_margin.add_child(inv_outer)

	var inv_left := VBoxContainer.new()
	inv_left.custom_minimum_size = Vector2(220, 0)
	inv_left.add_theme_constant_override("separation", 8)
	inv_outer.add_child(inv_left)

	var inv_preview_title := Label.new()
	inv_preview_title.text = "Player"
	inv_preview_title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	inv_left.add_child(inv_preview_title)

	inventory_preview_container = SubViewportContainer.new()
	inventory_preview_container.custom_minimum_size = Vector2(180, 210)
	inventory_preview_container.mouse_filter = Control.MOUSE_FILTER_IGNORE
	inv_left.add_child(inventory_preview_container)

	inventory_preview_viewport = SubViewport.new()
	inventory_preview_viewport.size = Vector2i(180, 210)
	inventory_preview_viewport.transparent_bg = false
	inventory_preview_viewport.own_world_3d = true
	inventory_preview_viewport.render_target_update_mode = SubViewport.UPDATE_DISABLED
	inventory_preview_container.add_child(inventory_preview_viewport)

	inventory_preview_root = Node3D.new()
	inventory_preview_root.name = "InventoryPreviewRoot"
	inventory_preview_viewport.add_child(inventory_preview_root)

	var inv_preview_env := WorldEnvironment.new()
	var inv_preview_env_res := Environment.new()
	inv_preview_env_res.background_mode = Environment.BG_COLOR
	inv_preview_env_res.background_color = Color(0.12, 0.14, 0.18, 1.0)
	inv_preview_env.environment = inv_preview_env_res
	inventory_preview_root.add_child(inv_preview_env)

	inventory_preview_camera = Camera3D.new()
	inventory_preview_camera.position = Vector3(0.0, 1.04, 2.0)
	inventory_preview_camera.look_at(Vector3(0.0, 1.0, 0.0), Vector3.UP)
	inventory_preview_root.add_child(inventory_preview_camera)

	var inv_preview_light := DirectionalLight3D.new()
	inv_preview_light.light_energy = 2.0
	inv_preview_light.rotation_degrees = Vector3(-24.0, -24.0, 0.0)
	inventory_preview_root.add_child(inv_preview_light)

	var inv_preview_fill := OmniLight3D.new()
	inv_preview_fill.light_energy = 0.8
	inv_preview_fill.position = Vector3(0.6, 1.2, 1.3)
	inventory_preview_root.add_child(inv_preview_fill)

	inventory_preview_model = null
	...
	inv_left.add_child(equipment_title)
	...
	inv_left.add_child(equipment_grid)
	equipment_slot_buttons.clear()
	for slot_name in ["Hat", "Shirt", "Jacket", "Pants", "Gloves", "Belt", "Shoes"]:
		var slot_box := VBoxContainer.new()
		slot_box.custom_minimum_size = Vector2(46, 62)
		slot_box.add_theme_constant_override("separation", 2)
		equipment_grid.add_child(slot_box)
		var slot_label := Label.new()
		slot_label.text = slot_name.substr(0, min(3, slot_name.length()))
		slot_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		slot_box.add_child(slot_label)
		var equip_slot := KZ_SlotButton.new()
		equip_slot.disabled = true
		slot_box.add_child(equip_slot)
		equipment_slot_buttons.append(equip_slot)

	var inv_right := VBoxContainer.new()
	inv_right.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	inv_right.add_theme_constant_override("separation", 8)
	inv_outer.add_child(inv_right)
	...
	inv_right.add_child(creative_panel)
	var creative_inner := VBoxContainer.new()
	creative_inner.add_theme_constant_override("separation", 8)
	creative_panel.add_child(creative_inner)
	...
	creative_inner.add_child(creative_tabs)
	...
	creative_inner.add_child(creative_nav)
	...
	creative_inner.add_child(creative_center)
	...
	inv_right.add_child(backpack_title)

	var inv_grid_center := CenterContainer.new()
	inv_grid_center.size_flags_vertical = Control.SIZE_EXPAND_FILL
	inv_right.add_child(inv_grid_center)
	...
	hint_label.text = "Cleaner inventory layout: preview, equipment, crafting, and backpack all fit in one screen."
	hint_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	hint_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
	inv_right.add_child(hint_label)
	...
	character_panel.offset_right = 430
	character_panel.offset_top = -248
	character_panel.offset_bottom = 248
	character_panel.mouse_filter = Control.MOUSE_FILTER_STOP
	root.add_child(character_panel)

	var character_bg := TextureRect.new()
	character_bg.anchor_right = 1.0
	character_bg.anchor_bottom = 1.0
	character_bg.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	character_bg.stretch_mode = TextureRect.STRETCH_SCALE
	character_bg.texture = load("res://assets/textures/ui/newscreen.png")
	character_bg.modulate = Color(1.0, 1.0, 1.0, 0.92)
	character_bg.mouse_filter = Control.MOUSE_FILTER_IGNORE
	character_panel.add_child(character_bg)

	var character_margin := MarginContainer.new()
	character_margin.anchor_right = 1.0
	character_margin.anchor_bottom = 1.0
	character_margin.add_theme_constant_override("margin_left", 12)
	character_margin.add_theme_constant_override("margin_top", 12)
	character_margin.add_theme_constant_override("margin_right", 12)
	character_margin.add_theme_constant_override("margin_bottom", 12)
	character_panel.add_child(character_margin)

	var character_hbox := HBoxContainer.new()
	character_hbox.add_theme_constant_override("separation", 12)
	character_margin.add_child(character_hbox)

	var preview_wrap := VBoxContainer.new()
	preview_wrap.custom_minimum_size = Vector2(292, 0)
	preview_wrap.add_theme_constant_override("separation", 6)
	character_hbox.add_child(preview_wrap)

	var logo_rect := TextureRect.new()
	logo_rect.texture = load("res://assets/textures/ui/KaisenCraftLogo1.png")
	logo_rect.expand_mode = TextureRect.EXPAND_FIT_WIDTH_PROPORTIONAL
	logo_rect.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	logo_rect.custom_minimum_size = Vector2(180, 84)
	preview_wrap.add_child(logo_rect)
	...
	character_preview_container.custom_minimum_size = Vector2(292, 326)
	...
	character_preview_viewport.size = Vector2i(300, 350)
	...
	preview_env_res.background_color = Color(0.12, 0.14, 0.18, 1.0)
	...
	preview_light.light_energy = 2.1
	preview_light.rotation_degrees = Vector3(-24.0, -28.0, 0.0)
	character_preview_root.add_child(preview_light)

	var preview_fill := OmniLight3D.new()
	preview_fill.light_energy = 0.9
	preview_fill.position = Vector3(0.7, 1.3, 1.6)
	character_preview_root.add_child(preview_fill)

	character_preview_model = null

	var character_controls_scroll := ScrollContainer.new()
	character_controls_scroll.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	character_controls_scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
	character_hbox.add_child(character_controls_scroll)

	var character_controls := VBoxContainer.new()
	character_controls.custom_minimum_size = Vector2(300, 0)
	character_controls.add_theme_constant_override("separation", 10)
	character_controls_scroll.add_child(character_controls)
	...
	var character_tabs := TabContainer.new()
	character_tabs.custom_minimum_size = Vector2(320, 380)
	character_controls.add_child(character_tabs)

	var body_tab := VBoxContainer.new()
	body_tab.name = "Body"
	body_tab.add_theme_constant_override("separation", 10)
	character_tabs.add_child(body_tab)

	var body_texture_row := HBoxContainer.new()
	body_texture_row.add_theme_constant_override("separation", 8)
	body_tab.add_child(body_texture_row)
	var body_texture_label := Label.new()
	body_texture_label.text = "Body Texture"
	body_texture_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	body_texture_row.add_child(body_texture_label)
	character_body_texture_option = OptionButton.new()
	for texture_name in ["Male 1", "Male 2", "Male 3", "Male 4"]:
		character_body_texture_option.add_item(texture_name)
	character_body_texture_option.item_selected.connect(Callable(self, "_on_character_body_texture_selected"))
	body_texture_row.add_child(character_body_texture_option)

	var height_box := VBoxContainer.new()
	height_box.add_theme_constant_override("separation", 4)
	body_tab.add_child(height_box)
	...
	character_height_value_label.text = "1.829m • 6ft 0in"
	height_head.add_child(character_height_value_label)
	character_height_slider = HSlider.new()
	character_height_slider.min_value = 1.60
	character_height_slider.max_value = 2.240
	character_height_slider.step = 0.005
	...
	build_label.text = "Body Width / Build"
	body_tab.add_child(build_label)
	var triangle_script: Script = load("res://ui/BodyShapeTriangle.gd") as Script
	if triangle_script != null:
		character_shape_triangle = triangle_script.new()
		if character_shape_triangle != null:
			character_shape_triangle.custom_minimum_size = Vector2(260, 220)
			character_shape_triangle.connect("shape_changed", Callable(self, "_on_character_shape_changed"))
			body_tab.add_child(character_shape_triangle)

	var skin_row := HBoxContainer.new()
	skin_row.add_theme_constant_override("separation", 8)
	body_tab.add_child(skin_row)
	var skin_label := Label.new()
	skin_label.text = "Skin Color"
	skin_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	skin_row.add_child(skin_label)
	character_skin_color_button = ColorPickerButton.new()
	character_skin_color_button.custom_minimum_size = Vector2(90, 32)
	character_skin_color_button.color_changed.connect(Callable(self, "_on_character_skin_color_changed"))
	skin_row.add_child(character_skin_color_button)

	var body_hint := Label.new()
	body_hint.text = "Body texture controls definition. Height changes vertical size. The triangle controls width and build feel."
	body_hint.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	body_tab.add_child(body_hint)

	var hair_tab := VBoxContainer.new()
	hair_tab.name = "Hair"
	hair_tab.add_theme_constant_override("separation", 10)
	character_tabs.add_child(hair_tab)

	var hair_editor_script: Script = load("res://ui/HairEditorPanel.gd") as Script
	if hair_editor_script != null:
		character_hair_editor_panel = hair_editor_script.new()
		if character_hair_editor_panel != null:
			hair_tab.add_child(character_hair_editor_panel)
			character_hair_editor_panel.connect("hair_data_changed", Callable(self, "_on_character_hair_panel_data_changed"))
			character_hair_editor_panel.connect("hair_color_changed", Callable(self, "_on_character_hair_panel_color_changed"))
	else:
		var hair_fallback := Label.new()
		hair_fallback.text = "Hair editor script could not be loaded."
		hair_tab.add_child(hair_fallback)
...
func _set_inventory_open(open: bool) -> void:
	...
	if open:
		...
		if inventory_craft_hint != null:
			inventory_craft_hint.visible = not creative_mode
		if creative_panel != null:
...
func _refresh_character_controls_from_player() -> void:
	...
	var body_preset: String = str(profile.get("body_preset", "male1")).to_lower()
	var height_meters: float = float(profile.get("height_meters", 1.8288))
	var width_scale: float = float(profile.get("width_scale", 1.0))
	var shape_weights: Array = profile.get("shape_weights", [0.33, 0.34, 0.33]) as Array
	var skin_tone: Color = _dict_color(profile.get("skin_tone", [0.93, 0.81, 0.69, 1.0]), Color(0.93, 0.81, 0.69, 1.0))
	if character_body_texture_option != null:
		var body_idx: int = 0
		match body_preset:
			"male2":
				body_idx = 1
			...
			_:
				body_idx = 0
		character_body_texture_option.select(body_idx)
	if character_height_slider != null:
		character_height_slider.set_value_no_signal(height_meters)
	if character_skin_color_button != null:
		character_skin_color_button.color = skin_tone
	if character_hair_editor_panel != null and character_hair_editor_panel.has_method("setup_from_profile"):
		character_hair_editor_panel.call("setup_from_profile", profile)
	if character_shape_triangle != null:
		if shape_weights.size() >= 3 and character_shape_triangle.has_method("set_weights"):
			character_shape_triangle.call("set_weights", float(shape_weights[0]), float(shape_weights[1]), float(shape_weights[2]), false)
		elif character_shape_triangle.has_method("set_width_scale"):
			character_shape_triangle.call("set_width_scale", width_scale)
...
func _update_character_value_labels() -> void:
	if character_height_value_label != null and character_height_slider != null:
		character_height_value_label.text = _meters_to_height_text(character_height_slider.value)
...
func _meters_to_height_text(meters: float) -> String:
	var total_inches: int = int(round(meters / 0.0254))
	var feet: int = total_inches / 12
	var inches: int = total_inches % 12
	return "%.3fm • %dft %din" % [meters, feet, inches]
...
func _apply_character_ui_changes() -> void:
	...
	var profile: Dictionary = player.get_character_appearance()
	if character_body_texture_option != null:
		match character_body_texture_option.selected:
			1:
				profile["body_preset"] = "male2"
			2:
				profile["body_preset"] = "male3"
			3:
				profile["body_preset"] = "male4"
			_:
				profile["body_preset"] = "male1"
	if character_height_slider != null:
		profile["height_meters"] = character_height_slider.value
		profile["height_scale"] = character_height_slider.value / 1.8288
	if character_shape_triangle != null:
		if character_shape_triangle.has_method("get_width_scale"):
			profile["width_scale"] = float(character_shape_triangle.call("get_width_scale"))
		if character_shape_triangle.has_method("get_weights_array"):
			profile["shape_weights"] = character_shape_triangle.call("get_weights_array")
	if character_skin_color_button != null:
		var c: Color = character_skin_color_button.color
		profile["skin_tone"] = [c.r, c.g, c.b, c.a]
	if character_hair_editor_panel != null:
		if character_hair_editor_panel.has_method("get_current_preset_id"):
			var preset_id: String = str(character_hair_editor_panel.call("get_current_preset_id"))
			profile["hair_preset_id"] = preset_id
			profile["hair_style"] = preset_id
		if character_hair_editor_panel.has_method("get_current_hair_data"):
			profile["hair_data"] = character_hair_editor_panel.call("get_current_hair_data")
		if character_hair_editor_panel.has_method("get_current_hair_color"):
			var hc: Color = character_hair_editor_panel.call("get_current_hair_color")
			profile["hair_color"] = [hc.r, hc.g, hc.b, hc.a]
	_update_character_value_labels()
	player.apply_character_appearance(profile)
	_refresh_character_preview()
	_refresh_inventory_preview()

func _on_character_hair_panel_data_changed(hair_data: Dictionary, preset_id: String) -> void:
	if player == null:
		return
	var profile: Dictionary = player.get_character_appearance()
	profile["hair_preset_id"] = preset_id
	profile["hair_style"] = preset_id
	profile["hair_data"] = hair_data
	player.apply_character_appearance(profile)
	_refresh_character_preview()
	_refresh_inventory_preview()

func _on_character_hair_panel_color_changed(color: Color) -> void:
	if player == null:
		return
	var profile: Dictionary = player.get_character_appearance()
	profile["hair_color"] = [color.r, color.g, color.b, color.a]
	player.apply_character_appearance(profile)
	_refresh_character_preview()
	_refresh_inventory_preview()
...
func _ensure_inventory_preview_model() -> void:
	if inventory_preview_root == null or inventory_preview_model != null:
		return
	var player_model_script: Script = load("res://player/PlayerModel.gd") as Script
	if player_model_script == null:
		return
	inventory_preview_model = player_model_script.new()
	if inventory_preview_model == null:
		return
	inventory_preview_root.add_child(inventory_preview_model)
	if inventory_preview_model.has_method("set_first_person_hidden"):
		inventory_preview_model.call("set_first_person_hidden", false)

func _refresh_inventory_preview() -> void:
	if inventory_preview_viewport == null or player == null:
		return
	_ensure_inventory_preview_model()
	if inventory_preview_model == null:
		return
	var app_script: Script = load("res://player/CharacterAppearance.gd") as Script
	if app_script == null:
	...
	if preview_profile != null and preview_profile.has_method("apply_dict"):
		preview_profile.call("apply_dict", player.get_character_appearance())
		if inventory_preview_model.has_method("set_appearance_profile"):
			inventory_preview_model.call("set_appearance_profile", preview_profile)
		elif inventory_preview_model.has_method("apply_profile"):
			inventory_preview_model.call("apply_profile")
	if inventory_preview_viewport != null:
		inventory_preview_viewport.render_target_update_mode = SubViewport.UPDATE_ONCE
...
func _on_character_body_texture_selected(_idx: int) -> void:
	_apply_character_ui_changes()

func _on_character_skin_color_changed(_color: Color) -> void:
	_apply_character_ui_changes()

func _on_character_hair_style_selected(_idx: int) -> void:
	_apply_character_ui_changes()

func _on_character_hair_color_changed(_color: Color) -> void:
	_apply_character_ui_changes()
...
func _on_character_shape_changed(_lean: float, _athletic: float, _bulky: float, _width_scale: float) -> void:
	_apply_character_ui_changes()
